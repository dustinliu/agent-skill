...
When creating a new skill from scratch, always run the `init_skill.py` script using `uv run`. The script conveniently generates a new template skill directory that automatically includes everything a skill requires, making the skill creation process much more efficient and reliable.
...
**Note:** Always use `uv run` to execute scripts to ensure all dependencies are properly managed.
